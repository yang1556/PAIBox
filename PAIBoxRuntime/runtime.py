            ...
            for i, oframe_info in enumerate(oframe_infos):
                ...
                if len(oframes) != 0:
                    ...
                    if not np.array_equal(indices, []):
                        ...
                        # )
                        print("oframe_info", oframe_info)
                        print("on_coord,", oframes_on_coord & (SFF.GENERAL_MASK - SFF.DATA_MASK))
            ...
            for onode in output_dest_info.values():
                ...
                for t in range(timestep):
                    for dest_on_coord in onode.values():
                        ...
                        # # print(dest_on_coord["tick_relative"])
                        # # addr_axon: [0-X] -> [0-X]*timestep
                        # dest_on_coord["addr_axon"] *= timestep
                        # dest_on_coord["tick_relative"] += [t]
                        dest_on_coord["tick_relative"] = [x + t * lcn for x in dest_on_coord["tick_relative"]]
                        print(dest_on_coord["tick_relative"])
